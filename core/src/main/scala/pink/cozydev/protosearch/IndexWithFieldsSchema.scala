...
/**  For a type `A`, a SearchSchema describes the fields of the document representation
  *  of `A`.
  */
class IndexWithFieldsSchema[A] private (
  ...
  def indexBldr(defaultField: String): List[A] => IndexWithFields[A] =
    docs =>
      IndexWithFields(
        MultiIndex(defaultField, fields.head, fields.tail: _*)(docs),
        docs,
      )

}
object IndexWithFieldsSchema {
  def apply[A](
      head: (String, A => String, Analyzer),
      tail: (String, A => String, Analyzer)*
  ): IndexWithFieldsSchema[A] =
    new IndexWithFieldsSchema[A](NonEmptyList(head, tail.toList))
}

object IndexWithFieldsExample {
  case class Doc(title: String, author: String)

  val analyzer = Analyzer.default

  val b = IndexWithFieldsSchema[Doc](
    ("title", (d: Doc) => d.title, analyzer),
    ("author", (d: Doc) => d.author, analyzer),
  )

  b.indexBldr("title")(List.empty)
}
