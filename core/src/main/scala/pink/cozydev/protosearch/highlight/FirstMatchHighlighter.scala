...
/**
 * Highlights the first case-insensitive occurrence of a query string within text, trimming long
 * text to a window around the match.
 *
 * If no match is found, the text is returned trimmed to `maxSize`. When the match occurs far into
 * the text, a nearby word boundary is used as the starting point for the returned fragment.
 *
 * @param formatter
 *   the formatter used to wrap matches with tags
 * @param maxSize
 *   maximum number of content characters (excluding tags) in the result
 */
case class FirstMatchHighlighter(
  ...
  /**
   * Returns `str` with the first case-insensitive occurrence of `queryStr` wrapped in formatter
   * tags, trimmed to a window of `maxSize` content characters. If `queryStr` is not found, returns
   * the trimmed text without highlighting.
   */
